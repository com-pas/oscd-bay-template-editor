...
describe('FunctionsLayer component', () => {
  let editor: FunctionsLayer;

  function getFunctionPixelCoordinates(
    _editor: FunctionsLayer,
    functionName: string
  ) {
  ...
  describe('with an SCL document with bays and functions', () => {
    ...
    it.skip('renders the function coordinates tooltip correctly when placing a function', async () => {
      ...
      fnRect?.dispatchEvent(
        new MouseEvent('click', { clientX: fx, clientY: fy, bubbles: true })
      );
      fnRect?.dispatchEvent(
        new MouseEvent('mousemove', { clientX: fx, clientY: fy, bubbles: true })
      );
      await editor.updateComplete;
      await visualDiff(
        editor,
        `functions-layer/#2 Render function coordinates tooltip.png`
      );
    });
  });
});
