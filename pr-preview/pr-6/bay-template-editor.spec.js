...
describe('Bay Template Editor Plugin', () => {
    ...
    describe('createFunction', () => {
        function setupElementWithDoc(xml) {
            const doc = new DOMParser().parseFromString(xml, 'application/xml');
            element.doc = doc;
            return doc;
        }
        function triggerAndCapture(selected, name = 'F1') {
            ...
            const editCall = dispatchSpy.args.find(args => args[0].type === 'oscd-edit-v2');
            expect(editCall, 'oscd-edit-v2 event was dispatched').to.exist;
            const { edit } = editCall[0].detail;
            const { parent, node } = edit;
            dispatchSpy.restore();
            return { parent: parent, fn: node };
        }
        ...
        it('adds Function to parent Bay for ConductingEquipment, with PowerSystemRelation', async () => {
            ...
            const psr = priv.getElementsByTagNameNS('http://www.iec.ch/61850/2019/SCL/6-100', 'PowerSystemRelations')[0];
            expect(psr).to.exist;
            const rel = psr.getElementsByTagNameNS('http://www.iec.ch/61850/2019/SCL/6-100', 'PowerSystemRelation')[0];
            expect(rel).to.exist;
