...
export function getFunctionCoordinates(doc, parent) {
    ...
    if (parent.tagName === 'Substation') {
        ...
        if (parentX && parentY) {
            ...
            const childWithCoords = Array.from(parent.children).find(el => getSLDAttributes(el, 'x') && getSLDAttributes(el, 'y'));
            if (childWithCoords) {
    ...
    function isOccupied(testX, testY) {
...
export function getProcessPath(element) {
    ...
    const pathParts = [startingElementName];
...
export function createPowerSystemRelationPrivate(doc, path) {
    const nsp = doc.documentElement.lookupPrefix(eTr6100Ns) ?? eTr6100NsPrefix;
    if (!doc.documentElement.lookupPrefix(eTr6100Ns)) {
        doc.documentElement.setAttributeNS(xmlnsNs, `xmlns:${eTr6100NsPrefix}`, eTr6100Ns);
    }
    const priv = doc.createElementNS(doc.documentElement.namespaceURI, 'Private');
    priv.setAttribute('type', eTr6100PrivType);
    const relations = doc.createElementNS(eTr6100Ns, `${nsp}:PowerSystemRelations`);
 ...
 */
export function getFunctions(element) {
    const { tagName } = element;
    if (tagName === 'Bay' ||
        tagName === 'VoltageLevel' ||
        tagName === 'Substation') {
        return Array.from(element.querySelectorAll(`:scope > Function:not(:has(> Private[type="${eTr6100PrivType}"]))`));
    }
    if (tagName === 'ConductingEquipment' || tagName === 'PowerTransformer') {
        const path = getProcessPath(element);
        const doc = element.ownerDocument;
        return Array.from(doc.querySelectorAll(`Function:has(> Private[type="${eTr6100PrivType}"] > PowerSystemRelations > PowerSystemRelation[relation="${path}"])`));
    }
    return [];
}
/**
 ...
 */
export function getSldSvgs(sldEditor) {
    const substationEditors = Array.from(sldEditor.shadowRoot?.querySelectorAll('sld-substation-editor') ?? []);
    return substationEditors.flatMap(se => {
        const svg = se.shadowRoot?.querySelector('svg#sld');
        return svg ? [svg] : [];
    });
}
