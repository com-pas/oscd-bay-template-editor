...
import { docWithBayAndFunctions, docWithoutFunctions, docWithFunctionWithoutBay, } from './functions-layer-testfiles.js';
  ...
  <Substation name="S1">
    ...
    describe('hover interaction', () => {
        ...
        it('calls onHoverFunction with function element on mouseenter', async () => {
            ...
            const functionGroup = element.shadowRoot?.querySelector('g.function');
            expect(functionGroup).to.exist;
            functionGroup.dispatchEvent(new MouseEvent('mouseenter', { bubbles: false }));
            await element.updateComplete;
            expect(onHoverFunctionSpy.calledOnce).to.be.true;
            const calledWith = onHoverFunctionSpy.firstCall.args[0];
        ...
        it('calls onHoverFunction with null on mouseleave', async () => {
            ...
            functionGroup.dispatchEvent(new MouseEvent('mouseenter', { bubbles: false }));
            await element.updateComplete;
            functionGroup.dispatchEvent(new MouseEvent('mouseleave', { bubbles: false }));
            await element.updateComplete;
        ...
        it('applies highlight styles on hover', async () => {
            ...
            expect(rect?.getAttribute('fill')).to.equal(SELECTED_PSR_HIGHLIGHT_STYLE.fill);
            expect(rect?.getAttribute('stroke')).to.equal(SELECTED_PSR_HIGHLIGHT_STYLE.stroke);
            expect(rect?.getAttribute('fill')).to.equal(SELECTED_PSR_HIGHLIGHT_STYLE.fill);
        });
    });
});
